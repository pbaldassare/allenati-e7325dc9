-- Migliorare la funzione smart_generate_sessions_with_weeks per gestire i conflitti
...
BEGIN
  ...
  -- This should already be done by the calling function, but we ensure it here
  ...
  IF NOT EXISTS (SELECT 1 FROM course_schedules WHERE course_id = _course_id AND is_active = true) THEN
