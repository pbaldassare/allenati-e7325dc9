-- Step 1 Final: Remove unique constraint on subscription_plans.name to allow gym-specific plans

-- 1. Drop unique constraint on subscription_plans name to allow same names for different gyms
ALTER TABLE public.subscription_plans DROP CONSTRAINT IF EXISTS subscription_plans_name_key;
...
-- 8. Create new subscription plans for each gym (only for plans without gym_id)
...
-- 10. Create unique constraint for subscription plans per gym (name + gym_id combination)
CREATE UNIQUE INDEX IF NOT EXISTS subscription_plans_name_gym_unique 
ON public.subscription_plans (name, gym_id) 
