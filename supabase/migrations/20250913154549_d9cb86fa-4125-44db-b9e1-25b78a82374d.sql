-- Add duration_weeks field to courses table
ALTER TABLE public.courses 
ADD COLUMN duration_weeks INTEGER NOT NULL DEFAULT 12;

-- Update existing courses to have 12 weeks duration and recalculate end_date
UPDATE public.courses 
SET 
  duration_weeks = 12,
  end_date = CASE 
    WHEN start_date IS NOT NULL THEN start_date + INTERVAL '84 days' -- 12 weeks * 7 days
    ELSE CURRENT_DATE + INTERVAL '84 days'
  END
WHERE duration_weeks IS NULL OR end_date IS NULL;

-- Set start_date for courses that don't have it
UPDATE public.courses 
SET start_date = CURRENT_DATE
WHERE start_date IS NULL;

-- Delete sessions beyond the 12-week limit for all courses
DELETE FROM public.course_sessions 
WHERE id IN (
  SELECT cs.id 
  FROM public.course_sessions cs
  JOIN public.courses c ON cs.course_id = c.id
  WHERE cs.session_date > c.end_date
);

-- Update the generate_course_sessions function to respect duration_weeks
CREATE OR REPLACE FUNCTION public.generate_course_sessions_with_duration(_course_id uuid, _start_date date DEFAULT NULL)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
AS $function$
