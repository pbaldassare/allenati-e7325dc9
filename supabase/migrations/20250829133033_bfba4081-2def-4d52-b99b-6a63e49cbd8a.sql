-- Miglioriamo il debugging e i permessi RLS per Combat Lab
...
-- 2. Miglioriamo la policy per user_gym_memberships per essere più permissiva nel debugging
...
-- 4. Creiamo una policy più permissiva per gli admin
CREATE POLICY "Admins can view all gym member subscriptions" 
ON user_subscriptions 
FOR SELECT 
USING (has_role(auth.uid(), 'admin'::app_role));

-- 5. Assicuriamoci che l'utente di Combat Lab abbia il ruolo corretto
-- Verifichiamo e aggiorniamo se necessario
DO $$
BEGIN
