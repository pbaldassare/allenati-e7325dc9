-- Fix the smart_generate_sessions_with_weeks function to return RECORD instead of JSONB
CREATE OR REPLACE FUNCTION public.smart_generate_sessions_with_weeks(_course_id uuid, _duration_weeks integer DEFAULT 12, _start_date date DEFAULT NULL::date)
 RETURNS RECORD
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- Return structured result as RECORD
  SELECT 
    true as success,
    session_count as sessions_created,
    deleted_count as sessions_deleted,
    affected_bookings as affected_bookings,
    calculated_start_date as start_date,
    calculated_end_date as end_date,
    _duration_weeks as duration_weeks,
    format('Generated %s sessions for %s weeks (%s to %s)', 
           session_count, _duration_weeks, 
           calculated_start_date, calculated_end_date) as message
  INTO result_record;
