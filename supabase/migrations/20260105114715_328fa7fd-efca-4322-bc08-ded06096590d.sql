-- Evita errori di duplicato durante la rigenerazione sessioni
-- (unique_course_session_datetime) rendendo l'inserimento idempotente.
    ...
    WHILE iter_date <= calculated_end_date LOOP
      IF EXTRACT(DOW FROM iter_date) = schedule_record.day_of_week THEN
        ...
        IF NOT is_exception THEN
          ...
          -- FIX: rende l'operazione idempotente se arrivano schedule duplicati
          ...
          ON CONFLICT ON CONSTRAINT unique_course_session_datetime DO NOTHING;

          GET DIAGNOSTICS inserted_rows = ROW_COUNT;
          session_count := session_count + inserted_rows;
        ELSE
