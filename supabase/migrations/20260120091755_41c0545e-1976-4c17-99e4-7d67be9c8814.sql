-- Fix: Riscrive la funzione smart_generate_sessions_with_weeks
-- Rimuove la logica UPDATE che causa "duplicate key violation" 
-- quando ci sono più schedule nello stesso giorno
...
BEGIN
    ...
    -- (No UPDATE of start_time - just cancel/delete sessions that don't match)
    ...
    LOOP
        ...
        -- If this session doesn't match any active schedule, remove it
        IF v_active_schedule_keys IS NULL OR NOT (v_schedule_key = ANY(v_active_schedule_keys)) THEN
            ...
            IF EXISTS (
                ...
                UPDATE course_sessions SET status = 'cancelled', updated_at = now()
                WHERE id = v_existing_session.id;
                
                -- Refund credits for affected bookings
                UPDATE bookings SET status = 'cancelled', cancellation_reason = 'Orario rimosso', cancelled_at = now()
                WHERE session_id = v_existing_session.id AND status IN ('confirmed', 'waitlist');
                
                v_booking_count := v_booking_count + (
                    SELECT COUNT(*) FROM bookings WHERE session_id = v_existing_session.id
                );
                v_cancelled_count := v_cancelled_count + 1;
                
                RAISE NOTICE '[SAFE-P] Cancelled session % with bookings', v_existing_session.id;
            ELSE
    ...
    LOOP
        ...
        WHILE v_session_date <= v_end_date LOOP
            IF EXTRACT(DOW FROM v_session_date) = v_schedule.day_of_week THEN
                -- Use INSERT with ON CONFLICT DO NOTHING to avoid duplicate key errors
    ...
    RETURN QUERY SELECT 
        ...
        v_deleted_count,
        v_booking_count,
        format('Generazione completata: %s sessioni create/aggiornate, %s cancellate, %s eliminate, %s prenotazioni impattate',
            v_created_count, v_cancelled_count, v_deleted_count, v_booking_count)::text;
END;
$function$;