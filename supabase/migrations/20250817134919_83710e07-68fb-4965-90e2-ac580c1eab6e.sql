-- Fix: Drop dependent trigger first, then recreate the complete multi-gym system

-- 1. Drop the existing trigger and function properly
...
-- 2. CREATE gym_credits table to replace profiles.current_credits
...
-- 3. ADD gym_id to existing tables if not exists
...
-- 4. UPDATE FUNCTIONS FOR MULTI-GYM SUPPORT
...
-- 5. New function to update gym credits
...
-- 6. DATA MIGRATION: Move existing credits from profiles to gym_credits
INSERT INTO public.gym_credits (user_id, gym_id, credits)
SELECT DISTINCT 
  p.user_id,
  ugm.gym_id,
...
-- 7. Update existing subscription plans to be gym-specific
...
-- 8. Update existing user_subscriptions to link to gym
...
-- 9. Update existing credits_transactions to link to gym
