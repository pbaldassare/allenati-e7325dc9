...
DECLARE
  ...
  _gym_name text;
BEGIN
  -- Caso 1: Proprietari che non hanno alcun record istruttore
  ...
  -- Caso 2: Proprietari che hanno record istruttore ma mancano instructor_gym_assignments per alcune palestre
  FOR missing_record IN 
    SELECT DISTINCT ugm.user_id as owner_user_id, ugm.gym_id as owner_gym_id, i.id as instructor_id
    FROM public.user_gym_memberships ugm
    JOIN public.instructors i ON ugm.user_id = i.user_id
  ...
  LOOP
    ...
    -- Create instructor_gym_assignment with owner privileges
    INSERT INTO public.instructor_gym_assignments (instructor_id, gym_id, has_owner_privileges, is_active)
    VALUES (missing_record.instructor_id, missing_record.owner_gym_id, true, true)
    ON CONFLICT (instructor_id, gym_id) DO UPDATE SET
      has_owner_privileges = true,
      is_active = true;
    ...
    user_name := COALESCE(_user_profile.first_name || ' ' || _user_profile.last_name, 'Nome Cognome');
    gym_name := _gym_name;
    action_taken := 'Added instructor assignment with owner privileges';
    RETURN NEXT;
  END LOOP;
